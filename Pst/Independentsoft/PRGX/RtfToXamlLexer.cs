...
namespace System.Windows.Documents
{
  ...
  internal class RtfToXamlLexer
  {
    ...
    internal RtfToXamlError Next(RtfToken token, FormatState formatState)
    {
      ...
      if (token.Type == RtfTokenType.TokenControl && token.Text.StartsWith("\\bin",StringComparison.InvariantCultureIgnoreCase))
      {
        binLength = token.Parameter;
      }
    ...
    internal void WriteImageData(Stream imageStream, bool isBinary, long binLength)
    {
      byte tokenChar = _rtfBytes[_rtfIndex];
      byte tokenNextChar;
    ...
    internal string ImageSourceForBinary(int binLength)
    {
      String retVal = string.Empty;
      if (_rtfIndex + binLength < _rtfBytes.Length)
      {
        retVal = Convert.ToBase64String(_rtfBytes, _rtfIndex, binLength, Base64FormattingOptions.InsertLineBreaks);
        _rtfIndex += binLength;
      }
      else
      {
    ...
    internal string ImageSourceForImageData()
    {
