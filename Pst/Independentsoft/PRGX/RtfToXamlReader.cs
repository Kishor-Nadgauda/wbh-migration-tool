...
namespace System.Windows.Documents
{
  ...
  internal enum RtfDestination
  {
    ...
    DestUPR = 11,
    DestField = 12,
    DestFieldInstruction = 13,
    DestFieldResult = 14,
    DestFieldPrivate = 15,
    DestShape = 16,
    DestShapeInstruction = 17,
    DestShapeResult = 18,
    DestShapePicture = 19,
    DestNoneShapePicture = 20,
    DestPicture = 21,
    DestPN = 22,
    DestUnknown = 23
  }
  ...
  internal class FontTableEntry
  {
    ...
    private static bool IsSymbolFont(string typefaceName)
    {
      ...
      if (typeface != null)
      {
        GlyphTypeface glyphTypeface = null; // typeface.TryGetGlyphTypeface();
  ...
  internal class RtfToXamlReader
  {
    ...
    internal RtfToXamlReader(byte[] rtfBytes)
    {
      _rtfBytes = rtfBytes;
      _bForceParagraph = false;
      ...
      while (rtfToXamlError == RtfToXamlError.None)
      {
        ...
        switch (token.Type)
        {
          ...
          case RtfTokenType.TokenPictureData:
            ProcessImage(_converterState.TopFormatState, token.Parameter);
            break;
        }
      }
    ...
    internal void ProcessImage(FormatState formatState, long binLength)
    {
      string contentType;
      ...
      if (_wpfPayload != null && contentType != string.Empty && !skipImage)
      {
        ...
        using (imageStream)
        {
          if (formatState.ImageFormat != RtfImageFormat.Wmf)
          {
            ...
            _lexer.WriteImageData(imageStream, formatState.IsImageDataBinary, binLength);
          }
          else
          {
            ...
            using (metafileStream)
            {
              ...
              _lexer.WriteImageData(metafileStream, formatState.IsImageDataBinary, binLength);
      ...
      else
      {
        ...
        //if (binLength > 0)
        //{
        //  formatState.ImageSource = _lexer.ImageSourceForBinary((int)binLength);
        //}
        //else
        //{
        //  // Skip the image data if the image type is unknown or WpfPayload is null
        //  formatState.ImageSource = _lexer.ImageSourceForImageData();
        //}
        ...
        //imageStringBuilder.Append("<BitmapImage ");
        //imageStringBuilder.Append("UriSource=\"");
        //imageStringBuilder.Append(formatState.ImageSource);
        //imageStringBuilder.Append("\" ");
        //imageStringBuilder.Append("CacheOption=\"OnLoad\" ");
        //imageStringBuilder.Append("/>");
        //imageStringBuilder.Append("</Image.Source>");
        ...
        if (binLength > 0)
        {
          _lexer.AdvanceForBinary((int)binLength);
        }
        else
        {
          // Skip the image data if the image type is unknown or WpfPayload is null
